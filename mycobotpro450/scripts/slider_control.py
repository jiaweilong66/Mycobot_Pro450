        ...
        try:
            ...
            try:
                if command.type == 'angles':
                    # 发送角度命令
                    mc.send_angles(command.data, 50)  # Pro450推荐速度
...
def main():
    ...
    print("选择控制模式:")
    print("  1: 滑块 → Gazebo仿真")
    print("  2: 滑块 → 真实 MyCobot Pro 450 (网络连接)")
    print("="*60)
    ...
    if mode == 1:
        rospy.loginfo("[slider_control]    - 控制Gazebo仿真机械臂")
    elif mode == 2:
        rospy.loginfo(f"[slider_control]    - 连接到真实机械臂 {PRO450_IP}:{PRO450_PORT}")
        rospy.loginfo("[slider_control]    - ⚠️  Gazebo机械臂不会移动，仅控制真实机械臂")
        rospy.loginfo("[slider_control]    - 夹爪角度自动映射到0-100范围")
